    ...
    with logging_pool.LoggingPool(max_games+1) as pool:
        while not terminated:
            ...
            if event["type"] == "terminated":
                break
            elif event["type"] == "local_game_done" or event["type"] == "local_game_ignored":
                busy_processes -= 1
                logger.info("+++ Process Free. Total Queued: {}. Total Used: {}".format(queued_processes, busy_processes))
            elif event["type"] == "challenge":
                chlng = model.Challenge(event["challenge"])
                if chlng.is_supported(challenge_config) and not chlng.is_ignore(challenge_config):
                    challenge_queue.append(chlng)
                    if (challenge_config.get("sort_by", "best") == "best"):
                        list_c = list(challenge_queue)
                        list_c.sort(key=lambda c: -c.score())
                        challenge_queue = list_c
                elif chlng.is_ignore(challenge_config):
                    continue
                else:
                    try:
...
def play_game(li, game_id, control_queue, engine_factory, user_profile, config, challenge_queue):
    ...
    if game.ignore(config):
        logger.info("--- {} Game ignored".format(game.url()))
        control_queue.put_nowait({"type": "local_game_ignored"})
        return
